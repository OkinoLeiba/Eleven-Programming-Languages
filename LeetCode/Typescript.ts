...
function removeElementTS(nums: number[] | string[] | null[], val: number): number {
    ...
    // return nums.map((v, i) => { if (nums[i] == val) { nums.splice(i, i, null) } }).length;
    return nums.filter(v => v != val).length;
}

let numsTS = [3, 2, 2, 3, 2, 0];
removeElementTS(numsTS, 3);
